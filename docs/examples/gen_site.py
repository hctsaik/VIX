...
<li>用 <code>flag_image_quality</code> 標出<b>像素層級</b>的品質問題(模糊 / 過曝過暗 / 長寬比離群),形成 <code>vixq:blurry</code> / <code>vixq:exposed</code> / <code>vixq:aspect</code> 工作清單 —— FiftyOne Enterprise「Data Quality」面板的離線替代,<b>免 golden</b>(看的是像素,不是標籤)。CLI 等價:<code>vix image-quality --tag</code>。一律 advisory:不刪檔、不改標,人工覆核後自行處理。</li>
</ol>
<div class="ref">完整 10 步走查(含可點的覆核佇列):見 <a href="../GUI_WALKTHROUGH.html">GUI_WALKTHROUGH.html</a>。</div>

<h2 class="sec">逐張覆核你的資料:覆核佇列</h2>
<div class="note"><b>一句話:</b>你先挑幾張「我確定標對了」的圖,VIX 就把「<b>最不像這些圖</b>」的排到最前面讓你先看 —— 一張圖越不像你的正確範例,越可能有問題,越該優先檢查。</div>

<p>會用到縮圖牆<b>上方工具列</b>那排 VIX 按鈕(橘色)。若視窗太窄、工具列空間不夠,它們會收進工具列上的橘色 <b>⌄</b> 選單裡:</p>
<table>
<tr><th>按鈕</th><th>作用</th></tr>
<tr><td>✓ <b>確認正確樣本(golden)</b></td><td>告訴 VIX「這幾張我檢查過、標註是對的」,當成比對用的<b>正確範例</b>(VIX 內部叫 golden)。</td></tr>
<tr><td>⊘ <b>標記誤報排除</b></td><td>這張其實沒問題、是系統<b>虛驚一場</b> → 按它把這張移出清單、之後不再出現。</td></tr>
<tr><td>🔄 <b>重新整理佇列</b></td><td>橘色實心按鈕。每次你確認了正確範例或排除了誤報,按它一下用最新資訊重排清單。</td></tr>
</table>

<ol class="steps">
<li><b>挑幾張正確的圖</b>:在 <code>Samples</code> 分頁勾選 5–10 張你確定標註正確、長相不同的圖。</li>
<li><b>按 ✓ 確認正確樣本(golden)</b>:工具列點這顆 → 跳出的小視窗按 <b>Execute(執行)</b>。這幾張就成了比對基準。</li>
<li><b>開啟覆核清單</b>:點工具列的 <b>VIX: 開啟覆核工作台</b> 按鈕(一鍵開出覆核佇列+弱點報告兩個面板;或在分頁列按 <b>+</b> 選 <b>VIX: 覆核佇列</b>)。<span class="ref">若它顯示「⚠ 覆核佇列尚未就緒」,代表你還沒確認任何正確範例 —— 回去做步驟 1、2。</span></li>
<li><b>按橘色的 🔄 重新整理佇列</b>。</li>
<li><b>逐張處理</b>:最上面是<b>最該先看</b>的圖(「風險」欄只是排序分數,高 = 最不像你的正確範例、該優先看,<b>不代表它一定錯</b>)。每列可:<b>看圖</b>(跳到該圖)、<b>確認→golden</b>(其實是對的 → 收進正確範例)、<b>誤報排除</b>(虛驚一場 → 移出)。</li>
<li>處理完一列它就消失;重複到清空。<b>你確認的正確範例越多,排序越準。</b></li>
</ol>

<figure><img src="img/queue_not_ready.jpg" alt="覆核佇列尚未就緒(含橘色重新整理按鈕)"><figcaption>還沒有正確範例時,清單會誠實顯示「尚未就緒」並停用 —— 而不是給你一份沒有依據的假清單。橘色的「🔄 重新整理佇列」按鈕就在警告下方。</figcaption></figure>

<div class="warn"><b>誠實提醒:</b>
<ul>
<li>沒有任何正確範例(golden)時,清單<b>停用</b>並顯示「尚未就緒」—— VIX 不會假裝有把握。</li>
<li>排序是「和正確範例多不像 + 模型信心多低 + 疑似標錯」的綜合,決定<b>看的順序</b>,不是「這張一定錯」的判決。</li>
<li>若你載入的校準是<b>別的資料集</b>算出來的,清單也會顯示「尚未就緒」—— 刻意如此,避免用錯基準排序。</li>
</ul></div>

<h2 class="sec">看 Embeddings 散點圖(DINO,免 Enterprise)</h2>
<div class="note">App 內建的 Embeddings 面板可以把資料畫成 2D 散點圖、框選(lasso)一群相似的物件 —— 很適合找離群與重複。<b>散點圖是物件級的:每個點 = 一個偵測框(YOLO label),不是整張圖</b>,所以你框選到的是「長得像的物件」,不會被整張圖的背景帶偏。<b>面板裡的「Create Embeddings」按鈕是 FiftyOne Enterprise 付費功能</b>,但<b>「看圖」本身在開源版就能用</b>:VIX 用你的 DINO 向量先把視覺化算好,你再開面板選它即可。</div>
<ol class="steps">
<li>工具列點 <b>VIX: 建立嵌入視覺化</b>(散點圖示)→ Execute。它會(必要時先算 DINO 嵌入,然後)用 UMAP 算好一份<b>物件級</b>視覺化(每個偵測框一個點),名稱 <code>vix_umap</code>。CLI 等價:<code>vix visualize</code>。</li>
<li>在分頁列按 <b>+</b> → 選 <b>Embeddings</b> 面板。</li>
<li>面板左上的 <b>Select brain key</b> 下拉選 <b>vix_umap</b> → 出現 2D 散點圖,每點對應一個偵測框。</li>
<li>用滑鼠 <b>框選(lasso)</b> 一群<b>物件框</b> → 對應的框會在格狀檢視中被選取 → 檢視那些是什麼物件(找離群、找同類、找重複)。</li>
</ol>
<div class="warn"><b>誠實提醒:</b>這張圖用的是你自己的 <b>DINOv2 向量</b>(非 Enterprise、非外部服務);第一次若還沒有嵌入會先離線算一次(會自動偵測 GPU/CPU)。</div>
<div class="ref">舊版圖解走查見 <a href="../EMBEDDINGS_HOWTO.html">EMBEDDINGS_HOWTO.html</a>。</div>
"""

similarity_body = """
<div class="note"><span class="tag b">Tier B</span> 在 App 裡點 <b>Similarity Search</b> 看到「<b>Upgrade to FiftyOne Enterprise</b>」?那個<b>內建面板</b>是付費功能、而且不吃你的 DINO。<b>不要用它。</b> 改用 VIX 自己的 <b>🔎 找相似物件</b> 按鈕 —— 它用你已算好的 <b>DINOv2 物件嵌入</b>,做<b>物件級</b>相似(找長得像的<b>瑕疵</b>,不是只找像的整張背景),完全離線、不需 Enterprise、不需外部向量資料庫。</div>
...
<h2 class="sec">② 用:選一張圖 → 「找相似物件」按鈕</h2>
<div class="warn"><b>不要用內建的「Similarity Search」面板</b>(<kbd>+</kbd> 開的那個)—— 那是 <b>FiftyOne Enterprise 付費</b>功能,會叫你升級,且不吃你的 DINO。請改用 VIX 自己的按鈕,它走你剛建立的 DINO 索引、完全離線。</div>
<ol class="steps">
<li>到 <b>Samples</b> 分頁,<b>選一張有框的影像</b>(格狀檢視點該圖左上角的勾選圈;或在展開的單張圖裡選一個 label,就會用那一個框當查詢)。</li>
<li>點工具列的 <b>🔎 VIX: 找相似物件</b> 按鈕。</li>
<li>App 立刻切到「<b>該物件的相似排序</b>」檢視(檢視列會出現 <code>ToPatches</code> + <code>SortBySimilarity</code>,最像的排最前面)。很適合:同一種瑕疵一次撈齊、找你可能漏標的同類、找重複。要還原就清掉檢視列的階段。</li>
</ol>
<figure><img src="img/sim_sorted.jpg" alt="按 VIX 找相似物件後的結果"><figcaption>選一張圖、按「找相似物件」後,App 依「這個坑洞長得多像」重排成物件 patches —— 檢視列顯示 <code>SortBySimilarity … vix_patch_sim</code>,全離線、用 VIX 的 DINOv2 物件嵌入,<b>沒有任何 Enterprise 升級提示</b>。</figcaption></figure>
