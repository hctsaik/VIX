...
<div class="note">App 內建的 Embeddings 面板可以把資料畫成 2D 散點圖、框選(lasso)一群相似的物件 —— 很適合找離群與重複。<b>面板裡的「Create Embeddings」按鈕是 FiftyOne Enterprise 付費功能</b>,但<b>「看圖」本身在開源版就能用</b>:VIX 用你的 DINO 向量先把視覺化算好,你再開面板選它即可。</div>
<ol class="steps">
<li>工具列點 <b>VIX: 建立嵌入視覺化</b>(散點圖示)→ Execute。它會(必要時先算 DINO 嵌入,然後)用 UMAP 算好一份視覺化,名稱 <code>vix_umap</code>。CLI 等價:<code>vix visualize</code>。</li>
<li>在分頁列按 <b>+</b> → 選 <b>Embeddings</b> 面板。</li>
<li>面板左上的 <b>Select brain key</b> 下拉選 <b>vix_umap</b> → 出現 2D 散點圖。</li>
<li>用滑鼠 <b>框選(lasso)</b> 一群點 → 只看選取 → 檢視那些是什麼影像(找離群、找同類、找重複)。</li>
</ol>
